...
# Secret #1
resource "aws_secretsmanager_secret" "weather_secret" {
  name = "weather_secret"
...
resource "aws_secretsmanager_secret_version" "super_secret_value" {
  secret_id     = aws_secretsmanager_secret.weather_secret.id
...
resource "aws_db_proxy" "historydb_proxy" {
  ...
  auth {
    auth_scheme = "SECRETS"
    secret_arn  = aws_secretsmanager_secret.weather_secret.arn
  }
...
resource "aws_iam_policy" "rds_proxy_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = aws_secretsmanager_secret.weather_secret.arn
    }]
  })
}
...
resource "aws_apigatewayv2_route" "options_proxy_weather" {
  api_id    = aws_apigatewayv2_api.weather_api.id
  route_key = "OPTIONS /weather/{location}"
  target    = "integrations/${aws_apigatewayv2_integration.weather_integration.id}"
}
...
resource "aws_apigatewayv2_route" "options_proxy_history" {
  api_id    = aws_apigatewayv2_api.weather_api.id
  route_key = "OPTIONS /history/{location}"
  target    = "integrations/${aws_apigatewayv2_integration.weather_integration.id}"
}
...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  viewer_certificate {
    cloudfront_default_certificate = true  # ✅ AWS-managed SSL
  }
...
resource "aws_s3_object" "config_json" {
  ...
  content = jsonencode({
    weatherApiUrl    = "${aws_apigatewayv2_api.weather_api.api_endpoint}/dev/weather",
    historyApiUrl = "${aws_apigatewayv2_api.weather_api.api_endpoint}/dev/history"
  })
